...
table11<-full_data%>%tab_cells(educ_f,agegr_f,region_f,male_f,as.factor(unemp_dur),as.factor(nationality_AUT),as.factor(health_condition),
...
tab1 <- tableby(group_nr ~ male_f + agegr_f+ educ_f+region_f+as.factor(unemp_dur)+as.factor(nationality_AUT)+as.factor(health_condition)+
                  as.factor(marginal_employment)+as.factor(German_ok), data=full_data)
summary(tab1, text=TRUE)
setwd(data_out)
capture.output(summary(tab1), file="Test_f.md")

## Convert R Markdown Table to LaTeX
require(knitr)
require(rmarkdown)
render("Test_f.md", pdf_document(keep_tex=TRUE))

#Strata plot
full_data<-full_data %>% group_by(strata1) %>%mutate(no_rows = length(personal_id))%>%ungroup()
full_data$no_rows[is.na(full_data$strata1)]<-NA

full_data$strata1 = with(full_data, reorder(strata1, no_rows, median))
full_data %>% filter(!is.na(strata1))%>%
  ggplot( aes(x=strata1))+geom_bar()+labs(x="",y="observations",title = "strata sizes")+
  theme_minimal()+theme(axis.text.x=element_blank())
ggsave("strataplot_f.png", path=data_out)
